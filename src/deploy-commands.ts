...
const {
  ...
  // 環境変数が足りない場合のエラーチェック
  if (!applicationId || !token || !GUILD_ID) {
  ...
  if (!guildId) {
    throw new Error("Missing GUILD_ID for the current environment");
  }
    ...
    const commandFiles = fs.readdirSync(commandsPath).filter(file => file.endsWith('.js') || file.endsWith('.cjs'));
        ...
        for (const file of commandFiles) {
            ...
            if (!commandModule.default) {
                console.warn(`Command module ${file} does not have a default export.`);
                continue;
            }
